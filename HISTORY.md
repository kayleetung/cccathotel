...
- **iPhone 標題字型不同**：Codex 為了「不依賴外部服務」拿掉 Google Fonts，但 iPhone 沒有內建繁中襯線字，標題就換成別的字型了。KK 用 iPhone 確認屬實。改成從 Windows 內建的 Noto Serif TC（OFL 1.1）切出標題用到的 140 個字，保留 400–500 粗細，約 50 KB，自己託管並 preload。scripts/build-font.cjs 負責重建；verify-release 會擋下字型沒涵蓋到的標題字。這需要安裝 Python 套件 fonttools、brotli，KK 已同意。
- **GA 只記瀏覽、沒記轉換**：在 script.js 加 line_click／phone_click／map_click／inquiry_open／inquiry_copy 事件，head 的 GA 原碼完全沒動（R5）。還需要 KK 到後台標記重要事件。
- **座標**：Codex 以「只有四位小數」為由拿掉 geo，理由不成立（四位小數約 11 m 精度）。但從 Google 商家檔案查到正確位置是 22.5959176, 120.3441024，舊座標 22.6183, 120.3536 偏北約 2.7 km，**所以舊座標確實是錯的**。拿掉的結論是對的，只是理由錯了。現在已補回正確座標，並加上 hasMap。
- Google 商家檔案評分 5.0，網站之前完全沒連過去。在社群連結加「Google 評論」；「開啟 Google 地圖導航」改連到商家頁，不再用地址搜尋。
- **改版時沒記錄的移除**：「目前營業中／休息中」小工具。判斷是移除正確，因為完全預約制下顯示「營業中」會引來沒預約的客人。已寫進 README，避免被加回來。
- **測試腳本**：原本寫死版號，並把 LocalBusiness 鎖在 7 月的舊值，導致每次發布都要先改測試才會過。現在改成從頁面讀版號、檢查 schema 和畫面是否一致。mutation 測試確認四種錯誤都會被擋下。
- **發布紀錄**：改成一個版本一列的簡化格式；純文件提交不用記錄。舊的長篇紀錄保留不動。
- 清理：刪掉日期試算取代加減按鈕後留下的 CSS、合併重複的 media query、更新過時的入口文件、修正簡體字。
- 檢視時發現內建瀏覽器面板在背景時，dialog 的 close 事件和 rAF 都不會觸發。用對照組確認是測試環境的問題，不是網站 bug。

## 2026-09-24 晚 — 導覽文案改為「看看環境」（2026.09.24-07）

-06 上線後，KK 認為「看看環境」比「店內環境」更符合網站整體語氣：口語、溫和，和首頁按鈕「先逛逛希希」同一種口吻。Claude 先前提過的缺點（導覽列其他項目都是名詞，這個是動詞）KK 已經知道，仍然選它。同樣改 3 處，其他內容不動。推送結果見 DEPLOYMENT_LOG。

## 2026-09-24 晚 — 導覽文案改為「店內環境」（2026.09.24-06）

KK 認為「旅館風景」聽起來規模很大，和小班制的實際樣子不符。討論過的選項有店內環境、看看環境、希希的家、店內實景。選「店內環境」的理由：飼主挑貓旅館時用的詞就是「環境」，FAQ 也寫「參觀環境」，四個字和其他導覽項目對齊。「希希的家」比較溫暖，但導覽列會和「關於希希」重複、點進去看到什麼也不夠明確。改了 3 處，大標題「每個角落，都能自在做自己。」保留。推送結果見 DEPLOYMENT_LOG。
...
- 公開 Netlify 網站已是 2026.09.23-01；00:01:20 核對 11 個資源全部 HTTP 200、內容一致、無 noindex 標頭。正式桌機與 390px 手機主要操作正常，無 console error/warn。
